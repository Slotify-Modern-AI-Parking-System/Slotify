...
def parking_lot_stats(request):
    if request.method == "GET":
        # 1. Total Revenue
        total_revenue = ParkingReservation.objects.aggregate(total=Sum('total_amount'))['total'] or 0

        # 2. Total Reservations (until today)
        total_reservations = ParkingReservation.objects.filter(created_at__date__lte=date.today()).count()

        # 3. Occupancy Rate (based on all parking lots average)
        occupancy_rates = []

        unique_locations = LicensePlateDetection.objects.filter(
            entry_time__isnull=False,
            entry_time__date__lte=date.today()
        ).values_list('location_id', flat=True).distinct()

        for location_id in unique_locations:
            detection_count = LicensePlateDetection.objects.filter(
                location_id=location_id,
                entry_time__isnull=False,
                entry_time__date__lte=date.today()
            ).count()

            total_slots = ParkingLotCoordinate.objects.filter(lotId_id=location_id).count()

            if total_slots > 0:
                occupancy = (detection_count / total_slots) * 100
                occupancy_rates.append(occupancy)

        occupancy_rate = round(sum(occupancy_rates) / len(occupancy_rates), 2) if occupancy_rates else 0

        # 4. Detection Accuracy (average similarity %)
        detection_accuracy = LicensePlateDetection.objects.aggregate(avg=Avg('detection_similarity_percentage'))['avg'] or 0
...
def revenue_trend_chart(request):
    if request.method == "GET":
        qs = ParkingReservation.objects.annotate(
        ...
        for entry in qs:
            ...
            month_name = calendar.month_name[month_num]  # e.g. 1 => 'January'
            revenue = float(entry['monthly_revenue'] or 0)
            if year not in data:
                data[year] = {}
            data[year][month_name] = revenue
...
@csrf_exempt
def reservations_trends(request):
    if request.method == "GET":
        qs = ParkingReservation.objects.annotate(
            year=ExtractYear('created_at'),
            quarter=ExtractQuarter('created_at'),
            weekday=ExtractWeekDay('created_at')  # 1=Sunday, 2=Monday, ..., 7=Saturday
        ).values('year', 'quarter', 'weekday').annotate(
            count=Count('id')
        ).order_by('year', 'quarter', 'weekday')
        ...
        for entry in qs:
            ...
            # Map ExtractWeekDay int to weekday name (Sunday=1 ... Saturday=7)
            # Let's shift so Monday=0 index: Monday=2 in ExtractWeekDay, so index=weekday-2 mod 7
            weekday_number = entry['weekday']
            # Adjusting to Monday=0 index
            weekday_index = (weekday_number + 5) % 7  
            weekday_name = calendar.day_name[weekday_index]  # Monday, Tuesday, ...

            if year not in data:
                data[year] = {}
            if quarter not in data[year]:
                data[year][quarter] = {"total": 0}

            # Add weekday count
            data[year][quarter][weekday_name] = data[year][quarter].get(weekday_name, 0) + count
            # Also add to total for quarter
            data[year][quarter]["total"] += count
