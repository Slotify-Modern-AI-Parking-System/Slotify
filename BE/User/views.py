...
def log_car_entry(request):
    if request.method == 'POST':
        try:
            ...
            if not plate_number or not lotid:
                return JsonResponse({'error': 'plate_number and lotid are required.'}, status=400)

            try:
                parking_lot = ParkingLot.objects.get(id=lotid)
            except ParkingLot.DoesNotExist:
                return JsonResponse({'error': 'Invalid lotid'}, status=404)
            ...
            return JsonResponse({
                ...
                'entry_time': detection.entry_time.isoformat()
            }, status=201)
