...
class PlateDetection:
    def __init__(self, camera_id, detection_duration=60):
        ...
        # Track first detection order
        self.first_detection_order = {}
        self.detection_order_counter = 0
        self.finalized_plate = None
    ...
    def add_detection(self, plate_number, confidence, text_size, bbox):
        ...
        # Track first detection order
        if plate_number not in self.first_detection_order:
            self.first_detection_order[plate_number] = self.detection_order_counter
            self.detection_order_counter += 1
        ...
        # Check if this plate should be finalized (first detected plate with 25 detections)
        if self.finalized_plate is None and self.all_detections_count[plate_number] >= 25:
            # Find the plate that was detected first among those with 25+ detections
            candidates = {p: count for p, count in self.all_detections_count.items() if count >= 25}
            if candidates:
                first_detected_plate = min(candidates.keys(), key=lambda p: self.first_detection_order[p])
                self.finalized_plate = first_detected_plate
                logging.info(f"🎯 FINALIZED PLATE: {first_detected_plate} (First detected with 25+ detections)")
    ...
    def get_most_detected_plate(self):
        # Return finalized plate if available, otherwise the most detected one
        if self.finalized_plate:
            return self.finalized_plate, self.all_detections_count[self.finalized_plate]
    ...
    def is_detection_complete(self):
        # Complete detection if we have a finalized plate or time is up
        if self.finalized_plate:
            return True
...
def detect_license_plates_for_duration(camera_id, duration=60):
    ...
    try:
        ...
        while not plate_detector.is_detection_complete():
            ...
            # Show if a plate is finalized
            if plate_detector.finalized_plate:
                cv2.putText(frame, f"FINALIZED: {plate_detector.finalized_plate}", (10, 270), 
        ...
        if most_detected_plate:
            ...
            if plate_detector.finalized_plate:
                print(f"🔒 FINALIZED PLATE: {plate_detector.finalized_plate} (First detected with 25+ detections)")
