...
#     handlers=[
#         logging.FileHandler('license_plate_exit_detection.log'),
...
#                 print(f"💾 License plate saved to exitDetection.txt")
...
class PlateDetection:
    def __init__(self, camera_id, detection_duration=60):
        ...
        self.plate_history = Counter()
        self.all_plates = Counter()
        self.finalized_plate = None  # Track the finalized plate
        ...
        self.recent_time_window = 8
        self.active_detection_timeout = 5
        self.finalization_threshold = 25  # First plate to reach 25 detections
    ...
    def add_detection(self, plate_number, confidence, text_size, bbox):
        # Check if we already have a finalized plate
        if self.finalized_plate is not None:
            return
        ...
        # Check if this plate has reached the finalization threshold
        if self.all_detections_count[plate_number] >= self.finalization_threshold:
            self.finalized_plate = plate_number
            logging.info(f"🎯 PLATE FINALIZED: {plate_number} reached {self.finalization_threshold} detections!")
    ...
    def get_most_detected_plate(self):
        # If we have a finalized plate, return it
        if self.finalized_plate is not None:
    ...
    def is_detection_complete(self):
        # Complete if we have a finalized plate or time is up
        if self.finalized_plate is not None:
            return True
...
def detect_license_plates_for_duration(camera_id, duration=60):
    ...
    print(f"⏱️  Detection Duration: {duration} seconds")
    print("🔧 Features: Stability scoring, temporal consistency, enhanced OCR")
    print("🎯 Finalization: First plate to reach 25 detections")
    print("=" * 60)
    ...
    try:
        ...
        while not plate_detector.is_detection_complete():
            ...
            if frame_count % process_interval == 0:
                ...
                for plate in plates:
                    ...
                    # Check if we've finalized a plate
                    if plate_detector.finalized_plate is not None:
                        break
                
                # Break out of main loop if we've finalized a plate
                if plate_detector.finalized_plate is not None:
                    break
            ...
            if plate_detector.finalized_plate is not None:
                cv2.putText(frame, f"FINALIZED: {plate_detector.finalized_plate}", (10, 30), 
        ...
        if most_detected_plate:
            if plate_detector.finalized_plate is not None:
                print(f"🎯 FINALIZED PLATE: {most_detected_plate} (reached 25 detections)")
            else:
                print(f"🎯 MOST DETECTED PLATE: {most_detected_plate}")
            print(f"📊 TOTAL DETECTION COUNT: {detection_count}")
            print(f"✅ TOTAL CONFIRMATIONS: {len(confirmed_detections)}")
